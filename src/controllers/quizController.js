...
const getUserQuizzes = async (req, res) => {
  try {
    ...
    const quizAttemptsRaw = await QuizAttempt.find({ userId: targetUserId })
      .populate({
        path: 'userId',
        select: 'username',
      })
    ...
    return res.status(200).json({ quizzes: quizAttempts });
  } catch (error) {
    console.error(error);
    res.status(500).json({ message: 'Failed to fetch user roadmaps' });
  }
};
export { getQuizById, submitQuiz, getQuizAttempt, getUserQuizzes };
