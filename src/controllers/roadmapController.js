...
const getRoadmap = async (req, res) => {
  ...
  try {
    ...
    //   buildRoadmapResponse(generatedRoadmap.roadmap)
    // );
    return res.status(200).json({
...
const getRoadmapsByUserId = async (req, res) => {
  try {
    ...
    if (!req.user || authenticatedUserId !== targetUserId) {
      return res
        .status(403)
    ...
    // console.log('User-progresses>>', progresses);
...
const deleteUserRoadmap = async (req, res) => {
  try {
    ...
    if (!roadmapId || !userId) {
      return res.status(400).json({ error: 'Missing roadmapId or userId.' });
    }

    // Remove roadmap reference from the user's document
    await User.findByIdAndUpdate(
      userId,
    ...
    await Promise.all([
      Progress.findOneAndDelete({ userId, roadmapId }),
      Notification.deleteMany({
        receiverUserId: userId,
        roadmapId,
      }),
      QuizAttempt.deleteMany({ userId, roadmapId }),
      ChatSession.deleteMany({
        userId,
        'context.roadmapId': roadmapId,
      }),
    ]);

    return res
      .status(200)
      .json({ success: true, message: 'Roadmap and related data deleted.' });
  } catch (error) {
    console.error('Error deleting roadmap:', error);
    return res
      .status(500)
      .json({ error: 'Server error while deleting roadmap.' });
  }
};
export {
  getAllRoadmaps,
