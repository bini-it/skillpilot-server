...
const deleteUserRoadmap = async (req, res) => {
  try {
    ...
    await Notification.deleteMany({ receiverUserId: userId, roadmapId });
    await QuizAttempt.deleteMany({ userId, roadmapId });
    await ChatSession.deleteMany({ userId, 'context.roadmapId': roadmapId });
